...
import type { QuizRound, AvatarConfig, QuizAssignment } from '../../types';
...
export const Quiz: React.FC<QuizProps> = ({ 
  ...
  const [assignmentAccuracy, setAssignmentAccuracy] = useState<number | null>(null);
  const [assignmentDoneDetecting, setAssignmentDoneDetecting] = useState(false);
  const [assignmentStars, setAssignmentStars] = useState(0);
  ...
  // --- ASSIGNMENT HANDLERS ---
  const startAssignment = (assignment: QuizAssignment) => {
    setSelectedAssignment(assignment);
    setAssignmentAccuracy(null);
    setAssignmentDoneDetecting(false);
    setAssignmentStars(0);
    setCurrentLevel(null);
  };

  const handleAssignmentAccuracyUpdate = (acc: number) => {
    setAssignmentAccuracy(acc);
  };

  const handleAssignmentDetectAI = () => {
    if (!selectedAssignment) return;
    const acc = assignmentAccuracy || Math.floor(75 + Math.random() * 21);
    setAssignmentAccuracy(acc);
    setAssignmentDoneDetecting(true);

    let stars = 1;
    if (acc >= selectedAssignment.targetAccuracy) {
      stars = 3;
    } else if (acc >= selectedAssignment.targetAccuracy - 15) {
      stars = 2;
    }
    setAssignmentStars(stars);
  };
  ...
  const handleCompleteAssignment = async () => {
    ...
    try {
      await ApiService.submitQuizResult({
        ...
        quiz_id: selectedAssignment.id,
        best_accuracy: assignmentAccuracy || 0,
        stars: assignmentStars
      });
      triggerConfetti();
      onQuizComplete(assignmentStars);
      onLoadAssignments();
  ...
  return (
    <div className="w-full font-nunito">
      ...
      {currentLevel === null && selectedAssignment === null && (
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_360px] gap-8">
          ...
          <div className="flex flex-col gap-6">
            {assignedQuizzes.length > 0 && (
              <div className="bg-white border border-slate-100 rounded-lg p-6 shadow-sm border-t-4 border-bisara-accent">
                ...
                <div className="flex flex-col gap-3">
                  {assignedQuizzes.map(quiz => {
                    const isActive = quiz.status === 'Aktif';
                    return (
                      <div 
                        key={quiz.id}
                        onClick={() => isActive && startAssignment(quiz)}
                        className={`p-3.5 border rounded-md flex flex-col gap-1 transition-all ${
                          isActive 
                            ? 'bg-blue-50 border-blue-200 hover:border-bisara-accent hover:shadow-sm cursor-pointer' 
                            : 'bg-slate-50 border-slate-100 opacity-60 cursor-not-allowed'
                        }`}
                      >
                        <div className="flex justify-between items-start">
                          <div className="text-sm font-black text-bisara-navy">"{quiz.word}"</div>
                        ...
                        <div className="flex justify-between text-[11px] text-slate-400 font-extrabold mt-1">
                          <span>Target: {quiz.targetAccuracy}% Acc</span>
                          <span>{quiz.assignedDate}</span>
                        </div>
                      </div>
                    );
                  })}
      ...
      {/* ==================== SCREEN: ASSIGNMENT CHALLENGE ==================== */}
      {selectedAssignment !== null && (
        <Card hoverEffect={false}>
          <div className="px-3 py-1 bg-blue-100 text-bisara-accent rounded-full text-xs font-black uppercase font-zain tracking-wider inline-block mb-4">
            Tugas Kuis Guru: "{selectedAssignment.word}"
          </div>
          <h3 className="font-zain text-4xl font-extrabold text-bisara-navy mb-4">Tantangan Kuis Isyarat Kamera</h3>
          <p className="text-slate-500 font-semibold text-base mb-8">
            Peragakan kata <strong>"{selectedAssignment.word}"</strong> di depan kamera sesuai target akurasi kelulusan setidaknya <strong>{selectedAssignment.targetAccuracy}%</strong>.
          </p>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-[380px_1fr] gap-8">
            ...
            {/* Guide Column */}
            <div className="flex flex-col gap-6">
              <div className="bg-white border border-slate-100 rounded-lg p-6 shadow-sm relative">
                <h4 className="text-xs text-slate-400 font-black uppercase mb-1">Target Penugasan</h4>
                <h3 className="font-zain text-4xl font-extrabold text-bisara-navy mb-4">
                  "{selectedAssignment.word}"
                </h3>
                <div className="text-xs font-extrabold text-slate-500 mb-2">Target Akurasi Kelulusan: {selectedAssignment.targetAccuracy}%</div>
                <div className="text-xs font-semibold text-slate-400">
                  Peragakan isyarat tangan secara tenang dan stabil agar model deteksi AI memindai pergerakan dengan akurat.
                </div>
              </div>

              {/* AI scoring outputs */}
              {assignmentDoneDetecting && (
                <div className="bg-white border border-slate-100 rounded-lg p-6 shadow-sm flex flex-col items-center gap-4">
                  <h4 className="text-xs font-black text-slate-400 uppercase">Hasil Penilaian AI</h4>
                  ...
                  <div className="flex gap-2">
                    {Array.from({ length: 3 }).map((_, i) => (
                      <span 
                        key={i} 
                        className={`text-4xl transition-all ${
                          i < assignmentStars ? 'text-bisara-yellow scale-110' : 'text-slate-200'
                        }`}
                      >
                        ★
                      </span>
                  ...
                  <div className="text-center">
                    <div className="text-2xl font-zain font-black text-bisara-navy">
                      Akurasi Terbaik: {assignmentAccuracy}%
                    </div>
                    <div className="text-xs text-slate-400 font-bold mt-1">
                      {assignmentStars === 3 ? '🎉 Sempurna! Kamu lulus target akurasi guru.' :
                       assignmentStars === 2 ? '👍 Bagus! Cukup untuk lulus kelayakan.' :
                       'Coba ulangi gerakan secara perlahan.'}
                    </div>
                  </div>
                </div>
              )}
            ...
            <div className="flex flex-col items-center gap-6 bg-slate-50 border border-slate-200 rounded-lg p-6">
              ...
              {/* Webcam Component */}
              <div className="max-w-[480px] w-full">
                <CameraPreview isQuizMode={true} onAccuracyUpdate={handleAssignmentAccuracyUpdate} />
              </div>

              {/* Interactive buttons */}
              <div className="flex gap-4">
                <button 
                  onClick={handleAssignmentDetectAI}
                  className="px-8 py-4 bg-bisara-pink text-white font-extrabold rounded-full shadow-[0_4px_15px_rgba(255,77,157,0.2)] hover:scale-105 active:scale-95 transition-transform"
                >
                  Mulai Deteksi AI
                </button>
                <button 
                  ...
                  disabled={!assignmentDoneDetecting}
                  className={`px-8 py-4 font-extrabold rounded-full transition-transform ${
                    assignmentDoneDetecting 
                      ? 'bg-bisara-accent text-white hover:scale-105 active:scale-95 shadow-[0_4px_15px_rgba(37,99,255,0.2)]' 
                      : 'bg-slate-200 text-slate-400 cursor-not-allowed'
